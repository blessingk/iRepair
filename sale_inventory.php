                                ...
                                <div id="new" class="tab-pane active">
                                    <section class="panel">
                                        <div class="panel-body bio-graph-info">
                                            <h1> Sell items <a class="pull-right btn btn-primary" href="inventory.php"><i class="icon_refresh"></i> Back</a></h1>
                                            <?php
                                            $ids=$_GET['inventory_number'];
                                            $qry="SELECT * FROM inventory WHERE inventory_number='$ids'";                                          //var_dump($query3);die();
                                            $res = $pdo->query($qry);
                                            if($res->rowCount() > 0) {
                                                while ($row = $res->fetch()) {
                                                    $id = $row['inventory_id'];
                                                ...
                                                if (isset($_POST['record'])) {
                                                    ...
                                                    if($quant <= $qnt) {
                                                        ...
                                                        try {
                                                            ...
                                                            echo '</script>';
                                                            $inse = "UPDATE inventory SET quantity='".$new_qnt."', total_p='".$new_p."', total_s='".$new_s."' WHERE inventory_number='$ids'";
                                                            $pdo->query($inse);
